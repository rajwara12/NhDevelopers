...
def handleSignup(request):
    if request.method == 'POST':
        ...
        messages.success(
            request, 'Your account has been created sucessfully. To Login Check your mail box and confirm mail')
        return redirect('index')
    return render(request, 'blog/index.html')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if not profile.is_verified:
                messages.success(
                    request, 'Check Your E-Mail and Verify your mail')
                return redirect('index')
...
def verify(request, auth_token):
    ...
    if profile_obj:
        if profile_obj.is_verified:
            ...
            messages.success(
                request, 'Your Mail is verified, Now you can Login')
            profile_obj.save()
            return redirect('index')
    else:
...
def forget_pass(request):
    if request.method == 'POST':
        ...
        if not User.objects.filter(username=username).first():
            ...
            messages.success(
                request, "We have sent an e-mail to your given email")
            return redirect('forget_pass')
    else:
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request, 'Your query has been submitted, Thanks for contact us')
        contact.save()
        send_conmail(email)
...
def postcomment(request):
    ...
    if request.method == 'POST':
        ...
        if parentSno == "":
            ...
            comment.save()
            messages.success(
                request, "Your Comment has been sucessfully commented")
        else:
...
def dashboard(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            # this code block won't run at all, because this block of code
...
def search(request):
    ...
    return render(request, 'blog/search.html',
                  {'allposts': allpostTitle, 'query': query})
